...
if args.function == 'pretrain':
    ...
    pretrain_config = trainer.TrainerConfig(
        ...
        num_workers=0,
        writer=writer)
    pretrain_instance = trainer.Trainer(model=model,train_dataset=pretrain_dataset,test_dataset=None,config=pretrain_config)
    pretrain_instance.train()
    ...
    finetune_dataset = dataset.NameDataset(data=finetune_data,pretraining_dataset=pretrain_dataset)
    if args.reading_params_path is not None:
        state_dict = torch.load(args.reading_params_path,map_location=device)
        model.load_state_dict(state_dict)
        tran_config = trainer.TrainerConfig(
            max_epochs=10,
            batch_size=256,
            learning_rate=args.finetune_lr,
            lr_decay=True,
        ...
        tran_config = trainer.TrainerConfig(
            ...
            num_workers=0,
            writer=writer)
    trainer_instance = trainer.Trainer(model=model,train_dataset=finetune_dataset,test_dataset=None,config=tran_config)
