...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trun_len = random.choice(range(4,int(self.block_size*7/8)))
        trun_data = data_element[:trun_len]
        mask_len = random.choice(range(int(round(trun_len*1/8)),int(round(trun_len*3/8))))
        mask_start_posi = random.choice(range(1,trun_len-mask_len))
        prefix = trun_data[:mask_start_posi]
        mask_content = trun_data[mask_start_posi:mask_start_posi+mask_len]
        suffix = trun_data[mask_start_posi+mask_len:]
