...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    theta_i = torch.tensor([[1/(10000**(2*(i-1)/dim)),1/(10000**(2*(i-1)/dim))] for i in range(1,dim//2+1)]) #(d/2,2)
    position = torch.arange(1,max_positions+1) #(max_position)
    theta_i = theta_i.unsqueeze(0) #(1,d/2,2)
    position = position.unsqueeze(1).unsqueeze(2) #(max_position,1,1) 
    rope_cache = theta_i*position #(max_positions, dim/2, 2)
    rope_cache[:,:,0] = torch.cos(rope_cache[:,:,0])
    rope_cache[:,:,1] = torch.sin(rope_cache[:,:,1])
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE  #x(b,nh,s,d)
    rope_cache = torch.view_as_complex(rope_cache) #(m,nh*d/2)
    batch_size = x.shape[0]
    dim = x.shape[3]
    seq_len = x.shape[2]
    rope_cache = rope_cache[:seq_len,:] #(s,nh*d/2)
    x = x.reshape(batch_size,seq_len,-1,2) #(b,s,nh*d/2,2)
    x = torch.view_as_complex(x) #(b,s,nh*d/2)
    rotated_x = x*rope_cache #(b,s,nh*d/2)
    rotated_x = torch.view_as_real(rotated_x).reshape(batch_size,-1,seq_len,dim) #(b,nh,s,d)
    pass
    ### END YOUR CODE ###
    return rotated_x
